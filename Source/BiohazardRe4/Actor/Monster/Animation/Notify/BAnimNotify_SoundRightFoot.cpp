...
#include "Actor/Monster/Animation/Notify/BAnimNotify_SoundRightFoot.h"
#include "Actor/Monster/Interface/BMonsterSoundInterface.h"

void UBAnimNotify_SoundRightFoot::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation, const FAnimNotifyEventReference& EventReference)
{
	if (MeshComp == nullptr)
	{
		return;
	...
	if (true == Owner->GetWorld()->LineTraceSingleByChannel(HitResult, Location + FVector(0.0f, 0.0f, 5.0f), Location + FVector(0, 0, -150), ECollisionChannel::ECC_GameTraceChannel2, LineTraceParams))
	{
		...
		case EPhysicalSurface::SurfaceType1:
			SoundInterface->PlayFootSound(EFootSoundType::Dirt, false);
			break;

		case EPhysicalSurface::SurfaceType2:
			SoundInterface->PlayFootSound(EFootSoundType::Rock, false);
			break;

		case EPhysicalSurface::SurfaceType3:
			SoundInterface->PlayFootSound(EFootSoundType::Wood, false);
			break;

		case EPhysicalSurface::SurfaceType4:
			SoundInterface->PlayFootSound(EFootSoundType::Water, false);
			break;

		case EPhysicalSurface::SurfaceType5:
			SoundInterface->PlayFootSound(EFootSoundType::Grass, false);
			break;
